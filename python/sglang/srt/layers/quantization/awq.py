...
if _is_cuda:
    ...
    warnings.warn(f"Only CUDA, CPU, HIP and XPU support AWQ currently.")
...
class AWQLinearMethod(LinearMethodBase):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        if _is_cpu:
            _amx_process_weight_after_loading(
                layer, ["qweight", "qzeros", "scales"], qweight_packed_method= "awq"
            )
        else:
...
class AWQMoEIntelAMXMethod(AWQMoEMethod):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        _amx_process_weight_after_loading(
            layer, ["w13_qweight", "w13_qzeros", "w13_scales"], qweight_packed_method="awq"
        )
        _amx_process_weight_after_loading(
            layer, ["w2_qweight", "w2_qzeros", "w2_scales"], qweight_packed_method="awq"
        )
