...
inline void clamp_sigmoid_and_mul(
  ...
  for (int64_t m = 0; m < m_size; ++m) {
    ...
    for (int64_t d = 0; d < 2 * BLOCK_N; d += 64) {
      float tmp_glu0[fVec::size()];  // 16
      float tmp_glu1[fVec::size()];
      float tmp_linear0[fVec::size()];
      float tmp_linear1[fVec::size()];
      ...
      for (int j = 0; j < 16  ; ++j) {
        ...
        tmp_glu0[j] = x[d + j * 2];
        //y0 [1,3,...31]
        tmp_linear0[j] = x[d + j * 2 + 1];
        //x1 [32,34,..62]
        tmp_glu1[j] = y[d  + j * 2];
        //y1 [33,35,..63]
        tmp_linear1[j] = y[d  + j * 2 + 1];
      }
...
void fused_experts_kernel_impl(
  ...
  at::parallel_for(0, MB * NB, 0, [&](int64_t begin, int64_t end) {
    ...
    for (int64_t i = begin; i < end; ++i) {
      ...
      const scalar_t* __restrict__ B0_bias = w1_bias + expert_id * 2 * N + nb0 * BLOCK_N;
      const scalar_t* __restrict__ B1_bias = w1_bias + expert_id * 2 * N + nb1 * BLOCK_N;
      ...
      if (use_brgemm) {
        ...
        const int64_t offset = offsets[mb];
        if (act_func == 2){
        // TODO: fix the acc issue here, not noly use brgemm
        tinygemm_kernel(
            /* A     */ A,
            /* B0    */ B0,
            /* B1    */ B1,
