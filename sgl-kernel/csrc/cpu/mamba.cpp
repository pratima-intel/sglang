...
void chunk_gated_delta_rule_kernel_impl(
    ...
    at::parallel_for(0, qk_num_head * global_num_chunk, 1, [&](int64_t begin, int64_t end) {
        ...
        int64_t h_qk = 0, c = 0;
        at::native::data_index_init(begin, h_qk, qk_num_head, c, global_num_chunk);
        for ([[maybe_unused]] auto z : c10::irange(begin, end)) {
            ...
            at::native::data_index_step(h_qk, qk_num_head, c, global_num_chunk);
        }
    });

    at::parallel_for(0, v_num_head * global_num_chunk, 1, [&](int64_t begin, int64_t end) {
        int64_t h = 0, c = 0;
        at::native::data_index_init(begin, h, v_num_head, c, global_num_chunk);
        int ompIdx = at::get_thread_num();
        int64_t offset = 0;
        ...
        for ([[maybe_unused]] auto z : c10::irange(begin, end)) {
            ...
            auto curr_k_cumdecay_reduced = k_cumdecay_reduced + h * global_total_seq_length * qk_head_size + c * chunk_size * qk_head_size;
            ...
            // Move to the next query
            at::native::data_index_step(h, v_num_head, c, global_num_chunk);
        }
    });
    ...
    at::parallel_for(0, batch_size * v_num_head, 1, [&](int64_t begin, int64_t end) {
        int64_t b = 0, h = 0;
        at::native::data_index_init(begin, b, batch_size, h, v_num_head);
        int ompIdx = at::get_thread_num();
        int64_t offset =
            /* k_transpose */ qk_head_size * chunk_size +
            /* v_pack */ chunk_size * v_head_size +
        ...
        for ([[maybe_unused]] auto z : c10::irange(begin, end)) {
            ...
            auto curr_q = q_pad + len_offset * qk_head_size + h_qk * global_total_seq_length * qk_head_size; // [num_chunk, chunk_size, EK]
            auto curr_k = k_pad + len_offset * qk_head_size + h_qk * global_total_seq_length * qk_head_size; // [num_chunk, chunk_size, EK]
            auto curr_v = v_beta_attn + h * global_total_seq_length * v_head_size; // [num_chunk, chunk_size, EV]
            auto curr_decay_mask = decay_mask + h * global_total_seq_length * chunk_size; // [num_chunk, chunk_size, chunk_size]
            auto curr_k_cumdecay_reduced = k_cumdecay_reduced + h * global_total_seq_length * qk_head_size; // [num_chunk, chunk_size, EK]
            auto curr_last_recurrent_state = final_state_data + b * final_state_StrideN + h * final_state_StrideH; // [EK, EV]
            auto curr_g_pad = g_pad + len_offset + h * global_total_seq_length; // [num_chunk, chunk_size]
            auto curr_core_attn_out = core_attn_out + len_offset * v_head_size + h * global_total_seq_length * v_head_size; // [num_chunk, chunk_size, EV]
            for (int64_t c = 0; c < num_chunk; c++) {
                ...
                auto k_cumdecay_i_reduced = curr_k_cumdecay_reduced + (chunk_offset + c) * chunk_size * qk_head_size; // [chunk_size, EK]
            ...
            // Move to the next query
            at::native::data_index_step(b, batch_size, h, v_num_head);
        }
    });
}
}  // anonymous namespace
